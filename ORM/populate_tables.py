...
from ORM.auto_grader_orms import MarkingScheme, Question, QuestionChoice, QuestionData, QuestionPaper, QuestionPaperData, SessionLocal, State
from datetime import datetime
from typing import Dict

def add_question_paper(form_data: QuestionPaperData):
    db = SessionLocal()
...
def create_ques_and_ques_choices(form_data: QuestionData):
    db = SessionLocal()
    ...
    for label, choice_text in form_data.choices.items():
        ...
        db.commit()
        if label == form_data.correct_choice_label:
